...
- `src/lib/constants.js` — pure data: embedded base64 logo reference, `ALTUNMAK_MODELS`, `COUNTRIES`/`COUNTRY_EN`/`COUNTRY_ALT`, `CITIES_TR`, seed arrays (`INIT_*`), `CURRENCIES`/`CUR_SYM`/`SALE_TYPES`/`DEFAULT_KDV_RATE`/`BACKUP_SCHEMA_VERSION`/`BACKUP_APP_TAG`/`SERVICE_TYPES`/`REPAIR_PLACES`/`PART_CATEGORIES`/`APP_VERSION`.
