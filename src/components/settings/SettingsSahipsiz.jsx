...
const TUR_ETIKET = { servis: "Servis", kalip: "Extra Kalıp", yedekParca: "Yedek Parça Satışı", yedekParcaBayi: "Yedek Parça Satışı (bayi)", odeme: "Ödeme/Kapora" };
const TUR_AUDIT = { servis: "servis", kalip: "kalip_satisi", yedekParca: "yedek_parca_satis", yedekParcaBayi: "yedek_parca_satis", odeme: "odeme" };
const bayiTuru = (tur) => tur === "yedekParcaBayi";

export const SettingsSahipsiz = ({
  rawCustomers = [], rawDealers = [], rawServices = [], rawPartSales = [], rawYedekParcaSatislar = [], rawPayments = [], rawParts = [],
  ...
  const liste = useMemo(() => sahipsizKayitlar({
    customers: rawCustomers, dealers: rawDealers, services: rawServices, partSales: rawPartSales, yedekParcaSatislar: rawYedekParcaSatislar, payments: rawPayments,
  }), [rawCustomers, rawDealers, rawServices, rawPartSales, rawYedekParcaSatislar, rawPayments]);

  const canliMusteriler = useMemo(() => rawCustomers.filter(c => !c.deletedAt), [rawCustomers]);
  const canliBayiler = useMemo(() => rawDealers.filter(d => !d.deletedAt), [rawDealers]);
  // Bağlama adayları: bayi satışı için bayiler, diğerleri için müşteriler (çöptekiler aday değil)
  const adaylar = useMemo(() => {
    const havuz = bagla && bayiTuru(bagla.tur) ? canliBayiler : canliMusteriler;
    const q = aramaNormalize(ara);
    const l = q ? havuz.filter(c => aramaNormalize(`${c.name || ""} ${c.serialNo || ""} ${c.model || ""} ${c.city || ""}`).includes(q)) : havuz;
    return l.slice(0, 30);
  }, [canliMusteriler, canliBayiler, bagla, ara]);
  ...
  const ozet = ({ tur, kayit: r }) => {
    ...
    if (tur === "yedekParca" || tur === "yedekParcaBayi") {
  ...
  const setterOf = (tur) => ({ servis: setServices, kalip: setPartSales, yedekParca: setYedekParcaSatislar, yedekParcaBayi: setYedekParcaSatislar, odeme: setPayments })[tur];
  ...
  const baglaUygula = (musteri) => {
    ...
    set(p => p.map(x => {
      ...
      if (tur === "yedekParcaBayi") return { ...x, aliciTipi: "bayi", dealerId: musteri.id }; // alıcı bayi
      if (tur === "yedekParca") {
    ...
    logAction({ serverPermissions, action: "duzenlendi", entity: TUR_AUDIT[tur], entityId: kayit.id, entityName: musteri.name, detail: { sahipsizBaglandi: true, eskiMusteriId: bagla.musteriId, eskiBayiId: bagla.bayiId } });
    showToast(`${TUR_ETIKET[tur]} kaydı "${musteri.name}" ${bayiTuru(tur) ? "bayisine" : "müşterisine"} bağlandı.`);
  ...
  const silUygula = () => {
    ...
    const set = setterOf(tur);
    if (!set) return;
    if (tur === "yedekParca" || tur === "yedekParcaBayi") yedekParcaGeriAl(kayit.id, setPartStock, setPartStockLog);
    ...
    logAction({ serverPermissions, action: "silindi", entity: TUR_AUDIT[tur], entityId: kayit.id, entityName: "(sahipsiz kayıt)", detail: { sahipsiz: true, eskiMusteriId: sil.musteriId, eskiBayiId: sil.bayiId } });
  ...
  return (
    <>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead>
              <tr style={{ textAlign: "left", color: "var(--n500, #64748b)", fontSize: 11, textTransform: "uppercase", letterSpacing: ".4px" }}>
                <th style={{ padding: "8px 10px" }}>Tür</th><th style={{ padding: "8px 10px" }}>Kayıt</th><th style={{ padding: "8px 10px" }}>Eski Müşteri / Bayi ID</th><th style={{ padding: "8px 10px", textAlign: "right" }}>İşlem</th>
              </tr>
            </thead>
            <tbody>
              {liste.map(item => (
                <tr key={`${item.tur}-${item.kayit.id}`} style={{ borderTop: "1px solid var(--n150, #f1f5f9)" }}>
                  ...
                  <td style={{ padding: "10px", fontFamily: "monospace", color: "var(--n500, #64748b)" }}>{String(bayiTuru(item.tur) ? item.bayiId : item.musteriId)}</td>
                  <td style={{ padding: "10px", textAlign: "right", whiteSpace: "nowrap" }}>
                    <span style={{ display: "inline-flex", gap: 6 }}>
                      <Btn small variant="ghost" onClick={() => { setBagla(item); setAra(""); }}><Icon name="edit" size={12} /> {bayiTuru(item.tur) ? "Bayiye Bağla" : "Müşteriye Bağla"}</Btn>
      ...
      {bagla && (
        <Modal title={bayiTuru(bagla.tur) ? "Bayiye Bağla" : "Müşteriye Bağla"} onClose={() => setBagla(null)}>
          <div style={{ fontSize: 13, color: "var(--n500, #64748b)", marginBottom: 12 }}>
            <b>{TUR_ETIKET[bagla.tur]}</b> · {ozet(bagla)} — hangi {bayiTuru(bagla.tur) ? "bayiye" : "müşteriye"} ait?
          </div>
          <div style={{ position: "relative", marginBottom: 10 }}>
            ...
            <input autoFocus value={ara} onChange={e => setAra(e.target.value)} placeholder={bayiTuru(bagla.tur) ? "Bayi adı veya şehir ara..." : "Firma adı, seri no veya model ara..."}
          ...
          <div style={{ maxHeight: 320, overflowY: "auto", border: "1px solid var(--n150, #f1f5f9)", borderRadius: 8 }}>
            {adaylar.length === 0 ? (
              <div style={{ padding: 16, textAlign: "center", color: "var(--n400, #94a3b8)", fontSize: 13 }}>{bayiTuru(bagla.tur) ? "Eşleşen bayi yok." : "Eşleşen müşteri yok."}</div>
            ) : adaylar.map(c => (
              <button key={c.id} type="button" onClick={() => baglaUygula(c)}
                ...
                <span style={{ color: "var(--n500, #64748b)", fontSize: 12 }}>{[c.model, c.serialNo, c.city].filter(Boolean).join(" · ")}</span>
              </button>
            ))}
          </div>
        </Modal>
      ...
      {sil && (
        <ConfirmDialog
          message={`${TUR_ETIKET[sil.tur]} kaydı (${ozet(sil)}) Çöp Kutusu'na taşınacak${sil.tur === "yedekParca" || sil.tur === "yedekParcaBayi" || sil.tur === "servis" ? "; kullanılan parçalar stoğa iade edilecek" : ""}. 30 gün içinde geri alabilirsiniz.`}
