    ...
    <>
      <Section title="Gönderilen E-postalar" icon="mail" collapsible defaultOpen>
        ...
        {sentEmailLog.length === 0 ? (
          <div style={{ padding: "24px 0", textAlign: "center", color: "var(--n400, #94a3b8)", fontSize: 13 }}>Henüz gönderilmiş e-posta yok.</div>
        ) : (
          <>
            ...
            <div style={{ border: "1px solid var(--n200, #e2e8f0)", borderRadius: 12, overflow: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead><tr style={{ background: "var(--n100, #f8fafc)" }}>
                  {["Tarih", "Kime", "Konu", "Durum", ""].map(h => <th key={h} style={{ padding: "8px 16px", textAlign: "left", fontSize: 11, fontWeight: 700, color: "var(--n600, #475569)" }}>{h}</th>)}
                </tr></thead>
                <tbody>
                  {sentEmailLogPaged.map((e, i) => (
                    <tr key={e.id || i} style={{ borderBottom: "1px solid var(--n150, #f1f5f9)" }}>
                      <td style={{ padding: "10px 16px", fontSize: 12, color: "var(--n500, #64748b)" }}>{e.timestamp ? new Date(e.timestamp).toLocaleString("tr-TR") : "—"}</td>
                      <td style={{ padding: "10px 16px", fontSize: 13, fontWeight: 600 }}>{e.to || "—"}</td>
                      <td style={{ padding: "10px 16px", fontSize: 13 }}>
                        {e.subject || "—"}
                        {Array.isArray(e.attachments) && e.attachments.length > 0 && (
                          <span style={{ marginLeft: 6, fontSize: 11, color: "var(--n400, #94a3b8)" }} title={e.attachments.map(a => a.filename).join(", ")}>
                      ...
                      <td style={{ padding: "10px 16px" }}>
                        {e.success ? (
                          <span style={{ fontSize: 11, fontWeight: 700, color: "var(--grn800, #065f46)", background: "var(--grnBg3, #d1fae5)", borderRadius: 6, padding: "2px 8px" }}>✓ Başarılı</span>
                        ) : (
                          <span style={{ fontSize: 11, fontWeight: 700, color: "var(--red800, #991b1b)", background: "var(--redBg2, #fee2e2)", borderRadius: 6, padding: "2px 8px" }} title={e.error || ""}>✗ Başarısız</span>
                        )}
                      </td>
      ...
      <Section title="Silinen E-postalar" icon="trash" collapsible>
        ...
        {deletedEmailLog.length === 0 ? (
          <div style={{ padding: "24px 0", textAlign: "center", color: "var(--n400, #94a3b8)", fontSize: 13 }}>Silinen e-posta yok.</div>
        ) : (
          <>
            {/* Tür filtresi */}
            {(() => {
            ...
            <div style={{ border: "1px solid var(--n200, #e2e8f0)", borderRadius: 12, overflow: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead><tr style={{ background: "var(--n100, #f8fafc)" }}>
                  {["Tarih", "Kime", "Konu", ""].map(h => <th key={h} style={{ padding: "8px 16px", textAlign: "left", fontSize: 11, fontWeight: 700, color: "var(--n600, #475569)" }}>{h}</th>)}
                </tr></thead>
                <tbody>
      ...
      {viewing && (
        <Modal title="E-posta İçeriği" onClose={() => setViewing(null)}>
          ...
          <div style={{ fontSize: 13, color: "var(--n900, #0f172a)", background: "var(--n100, #f8fafc)", border: "1px solid var(--n200, #e2e8f0)", borderRadius: 10, padding: 14, whiteSpace: "pre-wrap", maxHeight: 320, overflowY: "auto" }}>
