...
export function SettingsTwoFactor({ flash }) {
  ...
  const confirmEnable = async () => {
    ...
    setBusy(false);
    if (res?.ok) {
      // 2FA açılınca token_version arttı, diğer oturumlar düştü; bu oturumu ayakta tutmak için taze jetonu sakla
      if (res.data?.token) await window.appServer.updateToken(res.data.token);
      setRecovery(res.data.recovery || []); setStep("recovery"); setEnabled(true);
      flash("ok", "İki adımlı doğrulama açıldı. Diğer cihazlardaki oturumlar kapatıldı, tekrar giriş yapmaları gerekecek.");
    }
  ...
  const doDisable = async () => {
    ...
    if (res?.ok) {
      if (res.data?.token) await window.appServer.updateToken(res.data.token);
      setEnabled(false); setShowDisable(false); setDisablePw(""); flash("ok", "İki adımlı doğrulama kapatıldı.");
    }
  ...
  return (
    <Section title="İki Adımlı Doğrulama (2FA)" icon="lock">
      ...
      {enabled === null ? (
        ...
        <div style={{ background: "var(--n100, #f8fafc)", border: "1px solid var(--n200, #e2e8f0)", borderRadius: 12, padding: 18, maxWidth: 420 }}>
          <div style={{ fontSize: 13, fontWeight: 700, color: "var(--n700, #334155)", marginBottom: 10 }}>1) QR kodu authenticator uygulamanızla okutun</div>
          {qr && <img src={qr} alt="QR" style={{ width: 180, height: 180, display: "block", margin: "0 auto 10px", borderRadius: 8, background: "var(--surface, #ffffff)", padding: 6 }} />}
          <div style={{ fontSize: 11.5, color: "var(--n400, #94a3b8)", marginBottom: 14, wordBreak: "break-all" }}>Elle eklemek için anahtar: <b style={{ fontFamily: "monospace", color: "var(--n600, #475569)" }}>{secret}</b></div>
          <div style={{ fontSize: 13, fontWeight: 700, color: "var(--n700, #334155)", marginBottom: 8 }}>2) Uygulamadaki 6 haneli kodu girin</div>
          <input value={code} onChange={e => setCode(e.target.value)} inputMode="numeric" placeholder="6 haneli kod" maxLength={6}
            style={{ width: "100%", boxSizing: "border-box", padding: "10px 12px", fontSize: 18, letterSpacing: 6, textAlign: "center", border: "1px solid var(--n200, #e2e8f0)", borderRadius: 8, marginBottom: 10 }} />
          {err && <div style={{ fontSize: 12.5, color: "var(--red800, #991b1b)", marginBottom: 10 }}>✗ {err}</div>}
        ...
        <div style={{ background: "var(--ambBg, #fffbeb)", border: "1px solid var(--ambBr, #fde68a)", borderRadius: 12, padding: 18, maxWidth: 460 }}>
          <div style={{ fontSize: 14, fontWeight: 800, color: "var(--amb800, #92400e)", marginBottom: 6 }}>Yedek kurtarma kodları</div>
          <div style={{ fontSize: 12.5, color: "var(--amb800, #92400e)", marginBottom: 12, lineHeight: 1.6 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 14 }}>
            {recovery.map(c => <div key={c} style={{ fontFamily: "monospace", fontSize: 14, fontWeight: 700, color: "var(--n900, #0f172a)", background: "var(--surface, #ffffff)", border: "1px solid var(--ambBr, #fde68a)", borderRadius: 8, padding: "8px 10px", textAlign: "center" }}>{c}</div>)}
          </div>
        ...
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
            <span style={{ fontSize: 13, fontWeight: 700, color: "var(--grn800, #065f46)", background: "var(--grnBg3, #d1fae5)", padding: "6px 14px", borderRadius: 10 }}>✓ İki adımlı doğrulama açık</span>
          </div>
          {!showDisable ? (
            <Btn variant="ghost" onClick={() => { setShowDisable(true); setErr(""); setDisablePw(""); }}>Kapat</Btn>
          ) : (
            <div style={{ maxWidth: 340 }}>
              <div style={{ fontSize: 12.5, color: "var(--n500, #64748b)", marginBottom: 8 }}>Kapatmak için şifrenizi girin:</div>
