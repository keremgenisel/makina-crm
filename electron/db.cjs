...
CREATE TABLE IF NOT EXISTS yedek_parca_satis (
  id INTEGER PRIMARY KEY,
  dealer_id INTEGER,
  musteri_id INTEGER, aliciTipi TEXT,
  partId INTEGER, miktar INTEGER, birimFiyat REAL, currency TEXT,
  tarih TEXT, odendi INTEGER, faturaTipi TEXT,
  kargoFirma TEXT, kargoTakipNo TEXT, kargoTarih TEXT, kargoDurum TEXT,
...
CREATE TABLE IF NOT EXISTS yedek_parca_tahsis (
  id INTEGER PRIMARY KEY,
  satis_id INTEGER NOT NULL REFERENCES yedek_parca_satis(id),
  miktar INTEGER, customer_id INTEGER, serialNo TEXT, makinaSerbest TEXT, tarih TEXT, sort_order INTEGER
);
...
CREATE TABLE IF NOT EXISTS teklifler (
  ...
  takipKapali INTEGER,
  aliciTipi TEXT, dealer_id INTEGER, nihaiMusteriId INTEGER, uretilenKalemler TEXT
);
...
const TAHSILAT_TARIHI_COLUMN = [["tahsilatTarihi", "TEXT"]];
...
const SERVICES_DIS_FIRMA_COLUMNS = [["islemFirmaAd", "TEXT"], ["islemFirmaYetkili", "TEXT"], ["islemFirmaTel", "TEXT"], ["islemFirmaUlke", "TEXT"], ["islemFirmaSehir", "TEXT"], ["islemFirmaAdres", "TEXT"]];
...
const PART_SALES_KARGO_COLUMNS = [["kargoDurum", "TEXT"], ["kargoFirma", "TEXT"], ["kargoTakipNo", "TEXT"], ["kargoTarih", "TEXT"], ["kargoSorumlusu", "TEXT"], ["panoDusmeZamani", "TEXT"], ["panoGizli", "INTEGER"], ["olusturmaZamani", "TEXT"], ["fabrikaTeslim", "INTEGER"], ["teslimSekli", "TEXT"], ["teslimatFarkli", "INTEGER"], ["teslimatAd", "TEXT"], ["teslimatTel", "TEXT"], ["teslimatAdres", "TEXT"], ["teslimatUlke", "TEXT"], ["teslimatSehir", "TEXT"], ["teslimatIlce", "TEXT"], ["yontem", "TEXT"], ["vadeTarihi", "TEXT"], ["tahsilEdildi", "INTEGER"]];
...
// Spec 0006 (R10/R12/R13/R15): belgenin bayi alıcısı, nihai müşterisi ve üretilmiş alt kalem kimlikleri (JSON);
// üretilen yedek parça / Extra Kalıp kaydının kaynak belge ve alt kalem bağı.
const TEKLIFLER_URETIM_COLUMNS = [["aliciTipi", "TEXT"], ["dealer_id", "INTEGER"], ["nihaiMusteriId", "INTEGER"], ["uretilenKalemler", "TEXT"]];
const YEDEK_PARCA_TEKLIF_COLUMNS = [["teklifId", "INTEGER"], ["teklifKalemId", "TEXT"]];
const PART_SALES_TEKLIF_KALEM_COLUMN = [["teklifKalemId", "TEXT"]];
...
// Firma çalışma saatleri (JSON): mesai başlangıç/bitiş, çalışılan günler, molalar — servis işçilik
...
function populateAll(conn, data, skip = new Set()) {
  ...
  if (Array.isArray(data.partSales) && !skip.has("partSales")) {
    ...
    for (const p of data.partSales) {
      stmt.run({
        ...
        panoGizli: toInt(p.panoGizli), olusturmaZamani: p.olusturmaZamani ?? null, fabrikaTeslim: toInt(p.fabrikaTeslim), teslimSekli: p.teslimSekli ?? null,
  ...
  if (Array.isArray(data.yedekParcaSatislar) && !skip.has("yedekParcaSatislar")) {
    conn.prepare(`DELETE FROM yedek_parca_tahsis`).run();
    conn.prepare(`DELETE FROM yedek_parca_satis`).run();
    const stmt = conn.prepare(`
    ...
    // seçtiği rowid, sonraki açık id ile ÇAKIŞIYOR ve TÜM save transaction'ı geri alınıyordu
    // (UNIQUE constraint failed: yedek_parca_tahsis.id → servis+kargo hiç kaydedilmiyordu).
    const tStmt = conn.prepare(`
      INSERT INTO yedek_parca_tahsis (satis_id, miktar, customer_id, serialNo, makinaSerbest, tarih, sort_order)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `);
    for (const s of data.yedekParcaSatislar) {
      stmt.run({
        id: s.id, dealer_id: s.dealerId ?? null, musteri_id: s.musteriId ?? null, aliciTipi: s.aliciTipi ?? null,
        partId: s.partId ?? null, miktar: s.miktar ?? null,
        birimFiyat: s.birimFiyat ?? null, currency: s.currency ?? null, tarih: s.tarih ?? null,
        odendi: toInt(s.odendi), faturaTipi: s.faturaTipi ?? null,
        kargoFirma: s.kargoFirma ?? null, kargoTakipNo: s.kargoTakipNo ?? null, kargoTarih: s.kargoTarih ?? null,
...
function applyColumnMigrations(conn) {
  ...
  ensureColumns(conn, "services", TAHSILAT_TARIHI_COLUMN);
  ...
  ensureColumns(conn, "teklifler", TEKLIFLER_URETIM_COLUMNS);
  ensureColumns(conn, "yedek_parca_satis", YEDEK_PARCA_TEKLIF_COLUMNS);
  ensureColumns(conn, "part_sales", PART_SALES_TEKLIF_KALEM_COLUMN);
  ...
  ensureColumns(conn, "yedek_parca_satis", TAHSILAT_TARIHI_COLUMN);
  ...
  // Yedek parça stoğu hiçbir zaman eksiye düşmesin (sıfırda kalsın): eski sürümlerde servis/kit
  // tüketimi negatife düşürmüş olabilir → depolanan negatif satırları bir kerelik 0'a çek. İdempotent
  // (negatif yoksa no-op); okuma tarafı da Math.max(0,...) ile ayrıca güvence altına alır.
...
function readBlobFromDb() {
  ...
  const partStock = db.prepare(`SELECT * FROM part_stock`).all().map((s) => ({
    // Stok hiçbir zaman eksiye düşmez (sıfırda kalır) — okumada da güvence altına al: eski/negatif
    // kalmış satırlar 0 olarak döner (uygulama asla negatif görmez; sonraki kayıtta 0 kalıcılaşır).
    id: s.id, partId: String(s.part_id), miktar: Math.max(0, Number(s.miktar) || 0), notlar: s.notlar, sonGuncelleme: s.sonGuncelleme,
  }));
  ...
  const tahsisBySatis = new Map();
  for (const t of db.prepare(`SELECT * FROM yedek_parca_tahsis ORDER BY satis_id, sort_order`).all()) {
    if (!tahsisBySatis.has(t.satis_id)) tahsisBySatis.set(t.satis_id, []);
  ...
  const teklifler = db.prepare(`SELECT * FROM teklifler`).all().map(({ dealer_id, uretilenKalemler, ...t }) => ({
    ...t,
    customerId: t.customer_id,
    // Spec 0006: bayi alıcı ve üretilmiş alt kalem kimlikleri (JSON). Boş liste alanı taşınmaz (eski belge).
    ...(dealer_id != null ? { dealerId: dealer_id } : {}),
    ...(uretilenKalemler ? { uretilenKalemler: parseJsonCol(uretilenKalemler, []) } : {}),
