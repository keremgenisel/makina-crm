...
describe("Uygulamayı Kaldır — şifre doğrulaması", () => {
  ...
  it("kilit açıkken önce şifre sorar; hemen kaldırmaz", async () => {
    window.appLock = { status: () => Promise.resolve({ enabled: true }), verify: vi.fn() };
    await acEvetKaldir();
  ...
  it("yanlış şifre kaldırmaz, hata gösterir", async () => {
    window.appLock = { status: () => Promise.resolve({ enabled: true }), verify: () => Promise.resolve({ ok: false, error: "Şifre yanlış." }) };
    await acEvetKaldir();
  ...
  it("doğru şifre kaldırır", async () => {
    window.appLock = { status: () => Promise.resolve({ enabled: true }), verify: (pw) => Promise.resolve({ ok: pw === "1234" }) };
    await acEvetKaldir();
  ...
  it("kilit kapalıysa şifre sormadan doğrudan kaldırır", async () => {
    window.appLock = { status: () => Promise.resolve({ enabled: false }) };
    await acEvetKaldir();
    await waitFor(() => expect(uninstall).toHaveBeenCalled());
